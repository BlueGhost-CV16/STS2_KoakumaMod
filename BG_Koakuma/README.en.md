...
- Template Wiki (Rider-first): [Chinese Home](https://github.com/alkaid616/BG_Koakuma/wiki/Home) | [English Home](https://github.com/alkaid616/BG_Koakuma/wiki/Home-EN).
...
`dotnet new ritsulibmod -n MyMod` generates a project called `MyMod` and renames `BG_Koakuma`, sample class names, sample resource file names, resource folders, manifest names, namespaces, and localization IDs to match the new name.

### Option B: manual copy

1. Copy the whole directory and rename it to your mod name.
2. Edit `BG_Koakuma.json` and update `id`, `name`, `author`, and `description`.
3. Edit `BG_KoakumaCode/Entry.cs` and update `ModId`.
4. For a full rename, also update the project name and namespace in `.csproj`, `.sln`, and `project.godot`.
5. Rename the resource directory `BG_Koakuma/` to your `ModId`, then update the related `Entry.ResPath` paths in code.
...
> **`PackageReference` in `.csproj` only controls compile-time resolution; `dependencies` in `BG_Koakuma.json` is what the game loader checks at runtime. The template syncs the `STS2-RitsuLib` dependency version to the resolved NuGet version during build, but `min_game_version` still needs manual review.**
...
1. After build, confirm `dependencies[STS2-RitsuLib].version` in `BG_Koakuma.json` has been synced to the resolved `STS2.RitsuLib` version.
...
| Command | Behavior |
|---|---|
| `dotnet build .\BG_Koakuma.csproj` | Full build: compile + `CopyMod` + `ExportPCK` |
...
- **`CopyMod`**: copies the DLL and manifest to the game's `mods/BG_Koakuma` directory.
...
```text
BG_Koakuma/
├── BG_KoakumaCode/   # C# source
├── BG_Koakuma/       # Godot resources, localization, and placeholder scenes
├── BG_Koakuma.csproj
├── BG_Koakuma.json   # Mod manifest
├── project.godot
└── local.props.template
```

`res://BG_Koakuma/...` is the Godot/PCK resource path, mapping to the repository resource directory `BG_Koakuma/`, **not** to the C# namespace. When you create a project from the NuGet template, these directory names, file names, and namespaces are renamed consistently to match the new mod name.
...
| Property | Value |
|---|---|
| Type | `BG_KoakumaCharacter` |
| Expected ID | `BG_KOAKUMA_CHARACTER_BG_KOAKUMA_CHARACTER` |
| Starter deck | 4 × `BG_KoakumaStrike`, 4 × `BG_KoakumaDefend`, 1 × `BG_KoakumaRelic` |
...
|---|---|---|
| `BG_KoakumaStrike` (attack) | character card pool | `BG_KOAKUMA_CARD_BG_KOAKUMA_STRIKE` |
| `BG_KoakumaDefend` (skill) | character card pool | `BG_KOAKUMA_CARD_BG_KOAKUMA_DEFEND` |
| `BG_KoakumaRelic` | `BG_KoakumaRelicPool` | `BG_KOAKUMA_RELIC_BG_KOAKUMA_RELIC` |

### Static placeholder assets

**Images** (`res://BG_Koakuma/images/...`):

- `cards/BG_KoakumaStrike.png`, `cards/BG_KoakumaDefend.png`: sample card art.
- `relics/BG_KoakumaRelic.png`: sample relic icon.
- `characters/BG_Koakuma_character_*.png`: character icons, select art, map marker, and energy icons.

**Scenes** (`res://BG_Koakuma/scenes/characters/...`):

| Scene | Purpose | Placeholder structure |
|---|---|---|
| `BG_Koakuma_character.tscn` | Combat character | `%Visuals`, `%Bounds`, `%IntentPos`, `%CenterPos`, `%TalkPos` |
| `BG_Koakuma_energy_counter.tscn` | Energy counter | `%EnergyVfxBack`, `%Layers`, `%RotationLayers`, `%EnergyVfxFront`, `Label` |
| `BG_Koakuma_merchant.tscn` | Merchant | — |
| `BG_Koakuma_rest_site.tscn` | Rest site | `%ControlRoot`, `%SelectionReticle`, `%Hitbox`, `%ThoughtBubbleRight`, `%ThoughtBubbleLeft` |
| `BG_Koakuma_character_select_bg.tscn` | Character select background | — |
...
`BG_Koakuma.json` is the mod manifest. The game loader reads it at startup to identify the mod, check dependencies, and decide whether to load. Full example:
